...
> **Hermes agents:** Install the DKG CLI, run Hermes setup, and start the Hermes gateway:
> ```bash
> npm install -g @origintrail-official/dkg
> dkg init
> dkg hermes setup
> ```
> `dkg hermes setup` ensures DKG node config + daemon start + (optional) wallet funding + Hermes profile wiring with replace-by-default provider election (use `--preserve-provider` to opt out, `--no-start` / `--no-fund` for advanced flows). See the [adapter guide](packages/adapter-hermes/README.md) for details.
