#!/usr/bin/env bash
#
# Experiment A2: Optimized DKG — semantic tools, compact results, warm-up turn.
...
echo "=== Experiment A2: Optimized DKG (semantic tools + warm-up) ==="
...
This avoids expensive file exploration. Start with dkg_find_modules to locate relevant code, then dkg_file_summary to understand structure before reading.'
...
# =========================================================================
# Warm-up turn: teach the agent how the tools work on a trivial query
# =========================================================================
echo "--- Warm-up turn ---"
node "$CLAUDE_RUN" "$RESULTS_DIR/warmup.json" "$OPENCLAW_DIR" \
  -p \
  --output-format json \
  --max-turns 3 \
  --dangerously-skip-permissions \
  --append-system-prompt "$DKG_PREAMBLE" \
  "Use the DKG tools to answer: How many packages does OpenClaw have? List 5 channel extensions by path. Then use dkg_file_summary on the discord plugin file."
echo ""
