#!/usr/bin/env bash
#
# Experiment AB: 4 parallel Claude Code agents + SPARQL-only DKG (no semantic tools).
# Combines Exp-A's proven SPARQL cost-efficiency with Exp-B2's parallel throughput.
# DKG MCP server registered at user level with DKG_SPARQL_ONLY=1.
...
RESULTS_DIR="$EXPERIMENT_DIR/results/exp-ab"
...
echo "=== Experiment AB: Parallel Agents + SPARQL DKG ==="
echo "  OpenClaw: $OPENCLAW_DIR"
echo "  Results:  $RESULTS_DIR"
echo ""

# Exp-A proven preamble: SPARQL examples that worked well for cost-efficiency.
DKG_PREAMBLE='You have access to DKG MCP tools (dkg_query, dkg_publish). The OpenClaw codebase has been indexed into a code graph. BEFORE exploring files with Read/Grep/Glob, query the code graph using dkg_query to find relevant modules, functions, classes, and packages. Results come back as compact markdown tables. Examples:
...
This saves exploration tokens by letting you jump directly to relevant files.'
...
setup_worktree() {
  ...
  git -C "$OPENCLAW_DIR" worktree add "$worktree_dir" -b "exp-ab-${feature_id}" HEAD >/dev/null 2>&1 || true
  echo "$worktree_dir"
}
...
cleanup_worktrees() {
  ...
  if [[ -d "$worktree_base" ]]; then
    for wt in "$worktree_base"/*/; do
      ...
      git -C "$OPENCLAW_DIR" branch -D "exp-ab-${name}" 2>/dev/null || true
    done
    rm -rf "$worktree_base"
  fi
}
