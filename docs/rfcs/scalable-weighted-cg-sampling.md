...
1. **Settle-on-miss only, or also a keeper / epoch-due index?** Settle-on-miss alone heals *expiry* over-statement and is sufficient if (a) the live fraction of CGs stays high and (b) little value is future-dated (most publishes start now). Add the permissionless `settleMany` keeper if you expect significant dormancy *or* significant scheduled/extended value (under-statement, which settle-on-miss does **not** fix). Add the optional `epochDueCgs` index only if measurements show under-statement lag actually matters. **This hinges on expected dormancy and the share of future-dated value** — the main thing to decide, ideally from Phase-0 measurement.
2. **Fairness tolerance (decide before Test #6 can pass).** "Approximate weighting" needs a number. Define the acceptable steady-state deviation between draw frequency and true active weight (and/or a max wasted-retry rate), and make it the pass/fail bar for the drift-regime property test. Without it, "approximate enough" is undefined and there's no acceptance criterion for the lazy-settlement bet.
...
4. **Storage placement:** dedicated `CGWeightTreeStorage` (clean separation, reusable) vs fields on the fresh `RandomSamplingStorage`. Recommend dedicated.
