...
export class DKGAgent extends DKGAgentBase {
   ...
   */
  public async loadAgentsFromStore(): Promise<void> {
    const graph = DKGAgent.AGENT_SYSTEM_GRAPH;
    const DKG = 'https://dkg.network/ontology#';
   ...
   */
  public async autoRegisterDefaultAgent(): Promise<void> {
   ...
   * result ordering).
   */
  public async markDefaultAgent(agentAddress: string): Promise<void> {
    const graph = DKGAgent.AGENT_SYSTEM_GRAPH;
    const DKG = 'https://dkg.network/ontology#';
  ...
  public _pendingChatHandler: ChatHandler | null = null;
  public _pendingChatAcl: ChatAclCheck | null = null;
   ...
   */
  getWorkspaceSigningAgentForAddress(
  ...
  async getContextGraphAgentGateAddresses(contextGraphId: string): Promise<string[] | null> {
    const seen = new Set<string>();
    const agents: string[] = [];
   ...
   */
  async resolveOnChainAccessPolicyState(
    contextGraphId: string,
    opCtx?: OperationContext,
   ...
   */
  private isWireIdKeyedSubscription(localId: string): boolean {
   ...
   */
  async resolveWorkspaceRecipientsGated(
  ...
  async encryptWorkspacePayloadWithSenderKey(
  ...
  async createAndDistributeSwmSenderKeyEpoch(input: {
    contextGraphId: string;
    subGraphName?: string;
   ...
   */
  public async drainPendingSenderKeyForPeer(peerId: string, ctx?: OperationContext): Promise<number> {
    await this.loadSwmSenderKeyState();
   ...
   */
  async drainPendingSenderKeyForRecipients(
    recipients: readonly WorkspaceAgentRecipient[],
    ctx?: OperationContext,
  ): Promise<number> {
    if (this.pendingSenderKeyByAgent.size === 0) return 0;
   ...
   */
  public async handleSwmUpdate(data: Uint8Array, fromPeerId: string): Promise<Uint8Array> {
  ...
  public async handleSwmSenderKeyPackage(data: Uint8Array, fromPeerId: string): Promise<Uint8Array> {
  ...
  getLocalSigningAgentForAddress(agentAddress: string): (AgentKeyRecord & { privateKey: string }) | null {
    const checksum = ethers.getAddress(agentAddress);
    for (const record of this.localAgents.values()) {
  ...
  async loadSwmSenderKeyState(): Promise<void> {
  ...
  async saveSwmSenderKeyState(): Promise<void> {
    const path = this.swmSenderKeyStatePath();
    if (!path) return;
  ...
  async resolveWorkspaceGossipSigningAgent(
    contextGraphId: string,
  ...
  async encodeWorkspaceGossipMessage(
    contextGraphId: string,
    message: Uint8Array,
...
applyMixins(DKGAgent, [ContextGraphMethods, SwmHostModeMethods, PublishMethods, LifecycleSyncMethods]);
