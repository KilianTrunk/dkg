...
| `dkg_share` | `POST /api/shared-memory/write` | Directly write concise team-visible knowledge to SWM without staging a WM assertion. Prefer the WM assertion → promote flow for durable/canonical work. Both Hermes and OpenClaw expose this wrapper with parity contracts (free-text `content`, required `context_graph_id`, optional `sub_graph_name`). |
