...
| `dkg_share` | `POST /api/shared-memory/write` | Directly write concise team-visible knowledge to SWM without staging a WM assertion. Prefer the WM assertion → promote flow for durable/canonical work. Both Hermes and OpenClaw expose this wrapper with identical contracts: required `content` and `context_graph_id`, optional `sub_graph_name`. |
