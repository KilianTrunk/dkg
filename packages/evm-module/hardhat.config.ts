...
// deploy/archive/ (V8/V9 legacy stack archived per PRD §4.1). Also pinned
// in hardhat.node.config.ts; restated here so a paths.deploy grep across
// both configs surfaces the explicit single-root form.
config.paths = {
  ...(config.paths ?? {}),
  deploy: ['deploy'],
};
