 ...
 *
 * Compared to the legacy `mcp-server` loader (`packages/mcp-server/src/index.ts`):
 * the lazy `getClient: () => Promise<DkgClient>` getter is replaced by a
 * concrete `DkgClient`, and adapters now also receive the resolved
 * `DkgConfig` so they can honour the workspace's pinned project, agent URI,
 * and capture defaults without re-reading `.dkg/config.yaml`.
 */
