...
export class DashboardDB {
  ...
  private migrate(): void {
    ...
    if (version < 22) {
      // A3 (upstream): durable sync progress checkpoints. Kept at V22 rather
      // than V21 so that existing Guardian fork DBs — already at user_version
      // 21 with the Guardian audit pipeline tables — pick up this table on
      // upgrade instead of skipping the V21 gate.
      this.db.exec(`
