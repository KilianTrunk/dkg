...
// stores the KA's public quads in the PER-KA verified-memory graph
// `…/_verified_memory/{author}/{number}`, where `author` + `number` are
...
function perKaVerifiedMemoryGraph(contextGraphId: string, kaId: bigint): string {
  const number = kaId & ((1n << 96n) - 1n);
  const author = '0x' + (kaId >> 96n).toString(16).padStart(40, '0');
  return `did:dkg:context-graph:${contextGraphId}/_verified_memory/${author}/${number}`;
}
...
describe('DKGPublisher: no random publisher wallet without explicit key', () => {
  ...
  it('does not confirm adapter-managed updates from local metadata alone', async () => {
    ...
    // per-KA verified-memory graph (…/_verified_memory/{author}/{number}, keyed
    ...
    const vmGraph = perKaVerifiedMemoryGraph('1', 13n);
    const stored = await store.query(`
      SELECT ?p ?o WHERE {
        GRAPH <${vmGraph}> {
  ...
  it('keeps adapter-managed updates tentative when publisher attribution is unavailable', async () => {
    ...
    const vmGraph = perKaVerifiedMemoryGraph('1', 12n);
    const stored = await store.query(`
      SELECT ?p ?o WHERE {
        GRAPH <${vmGraph}> {
